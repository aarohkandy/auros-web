...
**Reviewed against DECISIONS.md: D40 (re-read 2026-09-20. `D36` makes a private repo for what has no CI to pay for and keeps the product repositories public for free Actions minutes — which is exactly what section 7's CI-cost row already records, and the site publishes no claim about repository visibility, so nothing moves. `D37` is about `./verify` reporting PASS on two suites it was not actually reading; it changes what an AGENT may trust, not what a reader is told, and no row here cites `verify` as its evidence. Both read, neither moves a published sentence. `D38` DID move one, and it is the only one that has: `copy.ts` detailsMono published `install_between 21:00-05:00` as a fact about the example recipe, and D38 measured that the field reaches the image as nothing — the update agent clears `OnCalendar=` on purpose so a fleet takes a rebuild inside check U1's window. The site now shows the field with the words `explain` prints, `RECORDED, NOT YET APPLIED`, and the row is in section 6 below. **Still open, and not mine:** `auros-web/worker/lib/explain.js` line 213 prints the old sentence into the pull request body a customer agrees to, while `auros-recipes/src/explain.ts` has been fixed — the two copies have drifted. `D39` adds `ids` and `tpm` columns to `compat.tsv`; the site publishes no compatibility row, so nothing moves. The earlier D35 read stands: D35 moves the INSTALLER to Go 1.25.14 after its vulnerability scan was found never to have run, and every "vulnerability" sentence on the site is about the OS image's CVE rebuild path, not the installer toolchain. `D40` runs SPEC §6D's last three sentences for the first time — Lighthouse ≥ 95 on mobile, and the no-JavaScript path — and fixes the three defects the first run found: a missing icon link, `role="log"` on an `<ol>`, and the configurator's one unlabelled `<select>`. **It moves no published sentence**, because the site makes no claim about its own Lighthouse score, its own accessibility conformance or its own page weight, and none of those would be claims worth making to a school. What it DOES move is an evidence column: section 6's row "The console works with JavaScript off" was measured by hand against one build, and is now held by `tools/nojs.mjs` on every build, with `tools/prove-red.mjs` watching that check go red. That row is updated; nothing a reader sees is different. D40's addendum — `/` measured 91 on a runner against §6D's 95, the Devanagari face narrowed from 267 KB to 75 KB, the first-boot figure moved onto a panel — also moves no published sentence: the site makes no claim about its own score, and the figure's words are unchanged; only the box they sit in is)** — 2026-09-20.
...
| An order opens a pull request in the public recipes repository | `pages/3-pricing.mdx`, `copy.ts` configurator | **SPEC** §6D. **DESIGN** — implemented in `auros-web/worker/lib/github.js`, which opens the pull request. | `2026-09-20` |
...
| Lockdown "is harder to undo on the machine, because it is compiled into the image rather than applied to it", and does **not** replace Group Policy's drive mappings, software deployment, proxy settings or printer identity | `migration/04-active-directory.md` | **DESIGN**, narrowed on 2026-09-20. Implemented in `auros-base/policy/kiosk/` (KDE Kiosk restrictions, `absent-binaries.list`, `assert.sh`). Rests on **MATRIX** check `S9` (kiosk implies no shell binaries in the image) and **DECISION** D3 (KDE Kiosk chosen because it makes lockdown provable). <!-- auros-allow: describes a superseded comparative claim in order to record that it was withdrawn --> The earlier wording asserted the lockdown was *stronger than* Group Policy, which is an unevidenced comparative claim against a product we have not measured, and false in scope: the same file lists four Group Policy capabilities that go away entirely. Tamper-resistance is the one property the design actually supports, so that is the only one claimed. `honesty-gate` rule `comparative-superiority` now fails the build on the pattern. Re-read once `S9` has recorded a pass. | `2026-09-20` |
...
| Claim | Where | Evidence | verified_on |
|---|---|---|---|
| A compatibility layer exists and we can install it | `migration/07-exe-compatibility-layer.md` | **UPSTREAM** for the layer itself (Bottles and WINE are on Flathub). **UNEVIDENCED for "we can install it":** **DECISION** D4.3 specifies a `compat_layer` recipe field, and on 2026-09-20 no such field exists in `auros-recipes/schema/recipe.schema.json` or `auros-recipes/src/`. Was marked DESIGN with no file behind it. **BUILD-REQ** on workstream B, or narrow the copy. | `2026-09-20` |
...
| Claim | Where | Evidence | verified_on |
|---|---|---|---|
| Files are copied to a disk that is not the system disk, then verified by file count and per-file hash, and only then is anything written | `migration/20-files.md`, `pages/1-landing.mdx`, `copy.ts` migrationSafety | **SPEC** §4.1 and §6C. **DESIGN** — implemented in `auros-installer/internal/safety/phase.go` (phase order), `auros-installer/internal/verify/verify.go` and `auros-installer/internal/safety/arm.go` (refuses to write while anything is unverified). Gate 3 is the evidence, and it requires 100 clean runs and 20 clean aborts; it has not passed. | `2026-09-20` |
...
| The installer reads the machine's actual installed-programs list and requires acknowledgement | `migration/01-windows-programs.md`, `copy.ts` migrationSafety | **SPEC** §6C.2. **DESIGN** — `auros-installer/cmd/auros-migrate/main.go` prints the installed-programs list (`printDisclosure`) and stops without `--i-understand-programs-do-not-migrate`. | `2026-09-20` |
| Wi-Fi network names and keys, printers, default printer and account name are **not** carried over | `migration/23-wifi-credentials.md`, `24-printers.md`, `25-account-name.md`, `09-print-management-clients.md`, `copy.ts` migrationSafety | **Absence of code.** No Wi-Fi, printer or account-name export exists anywhere in `auros-installer/` (SYSTEM-REVIEW §2.8: a grep for wlan, netsh, wifi, EnumPrinters and printer finds none). Until 2026-09-20 this row said they WERE carried over, marked DESIGN, and its evidence was "SPEC §4.2 and §6C.1 name exactly these" — the promise cited as proof of itself. SPEC still requires them; the site says what the installer does. The §4.2 disclosure in `auros-installer/cmd/auros-migrate/main.go` is corrected to match on branch `agent/honesty`. Statement against interest. | `2026-09-20` |
| The restore re-verifies on first boot and reports the file count on the desktop, and shows any discrepancy rather than swallowing it | `migration/20-files.md` | **SPEC** §6C, Linux side. **MATRIX** check `R1`, which fails on any discrepancy that is swallowed rather than shown. **UNEVIDENCED:** on 2026-09-20 no restore code is on `auros-installer` main; it exists only on the unmerged branch `feat/linux-restore` (SYSTEM-REVIEW §2.6). Was marked DESIGN with no file behind it. | `2026-09-20` |
...
| Files locked by Windows are listed as "in use and not copied" rather than dropped quietly | `migration/20-files.md`, `copy.ts` migrationSafety | **DECISION D14**: shadow-copy tooling is unavailable on current Windows client editions, so backup semantics are the default path and the residue is quarantined and reported. **DESIGN** — `auros-installer/internal/copyengine/copy.go` records locked files as `ReasonLocked` in `auros-installer/internal/quarantine/quarantine.go`. | `2026-09-20` |
...
| The archive stays on the customer's premises and is never uploaded | `faq/5-do-you-see-my-data.md`, `pages/1-landing.mdx` | **SPEC** §6C (destination is a local disk). Architectural: there is no upload path in the design. **DESIGN** — `auros-installer/internal/safety/destination.go` resolves a local destination, and nothing under `auros-installer/cmd/` or `auros-installer/internal/` imports a network package (checked 2026-09-20). | `2026-09-20` |
...
| A recipe may add and remove, and may never change the base, the kernel or a package version | `layers/3-stratum-1.md`, `pages/6-how-it-works.mdx` | **SPEC** §3, enforced by the schema validator (PLAN B1). **DESIGN** — `auros-recipes/schema/recipe.schema.json`, `auros-recipes/src/validate.ts` and `auros-recipes/schema/validate.py`. | `2026-09-20` |
...
| A security fix is one rebuild, and downstream follows without human action | `layers/4-stratum-2.md`, `pages/6-how-it-works.mdx` | **SPEC** §3. **DESIGN** — `auros-recipes/.github/workflows/propagate.yml` polls for a moved base. Gate 2 is the evidence, and **BLOCKED.md B2** and **DECISION D18** record that instant cross-repo propagation needs a credential the project does not yet have. The copy says "automatically", which a 15-minute poll satisfies. Acceptable. Do not upgrade the wording to "immediately". | `2026-09-20` |
| Images are signed, and a machine will not install an unsigned image | `faq/2-who-has-root.md`, `layers/4-stratum-2.md`, console sample | **SPEC** §6A. **MATRIX** checks `S8` (signature valid and discoverable) and `U4` (offered an unsigned or wrongly-signed image, the upgrade exits non-zero and the booted digest is unchanged). **DECISION D8** records that this does not come for free by deriving from upstream and that we ship the policy ourselves, which is why `U4` is the check that matters. **DECISION D17** records that the signature format is a pinned, time-boxed upstream risk. **DESIGN** — `auros-base/signing/policy.json` and `auros-base/update-agent/greenboot/check/wanted.d/50-signature-enforcement.sh`. | `2026-09-20` |
| Images are signed with a key we hold (cosign, keyed), not keyless | `copy.ts` build-console sample (`check S8`) | **DECISION D8, D17.** `auros-base/.github/workflows/build.yml` runs `cosign sign --key`, and `auros-base/signing/policy.json` pins a public key. The private key is a real risk, recorded in `auros-base/signing/RISKS.md`. Until 2026-09-20 this row read "There is no private key to lose; signing is keyless", citing SPEC §6A's original wording, which D8 superseded, and the sample line printed `cosign keyless`. Both were false. | `2026-09-20` | <!-- auros-allow: quotes the withdrawn keyless wording to record that it was withdrawn -->
| The previous image stays on disk, and a machine that fails to reach a login prompt twice rolls back by itself | `faq/3-update-breaks-a-machine.md`, `layers/1-surface.md` | **SPEC** §6A, in those words. **MATRIX** checks `U2` (two deployments retained) and `U3` (repeated failed boots return to the previous deployment). **DECISION D9**: the rollback mechanism is absent from upstream and we install it deliberately, and the storage backend is constrained because of it. **DESIGN** — `auros-base/update-agent/greenboot/` and `auros-base/build/30-update-agent.sh`. | `2026-09-20` |
...
| An image is published only after booting in a VM and passing the full matrix, and the publish step reads the ledger with no override flag | `faq/3-update-breaks-a-machine.md`, `pages/6-how-it-works.mdx`, console legend | **SPEC** §4.3, §8. **MATRIX** header states the gate cannot be overridden. PLAN §3.2 notes the real gate is four layers, of which the CI publish step reading the ledger is the first. **DESIGN** — `tools/gate.mjs` reads `attest/passed-digests.tsv`. | `2026-09-20` |
| Removed packages are deleted from the image's filesystem, and are **still downloaded** | `pages/1-landing.mdx`, `faq/7-updates-one-uplink.md`, `copy.ts` removeBlockNote | SYSTEM-REVIEW §2.9. The customer's image is built with plain `podman build` and pushed with `podman push` (`auros-recipes/.github/workflows/build-recipe.yml`); the prune is a new layer on top of the base. The flatten D2/D11 describe runs only in `auros-base/.github/workflows/build.yml`, on an image no customer pulls. Statement against interest. Until 2026-09-20 this row said removed packages are bytes the customer never downloads, marked DESIGN on the strength of D2. | `2026-09-20` |
| The removal report names every removed package with a measured size | `pages/1-landing.mdx`, `layers/3-stratum-1.md`, tier includes | **MATRIX** check `S5` requires a real measured byte count and forbids counting recipe lines. Strong evidence for the shape of the claim. **DESIGN** for its existence — `auros-recipes/src/prune.ts` defines the report's shape; the measured numbers come from a built image. | `2026-09-20` |
...
| Machines pull and stage in the background, apply on restart, and are not synchronised | `faq/7-updates-one-uplink.md`, `layers/1-surface.md` | **SPEC** §6A for pull-on-boot and staging. **DESIGN** — `auros-base/update-agent/systemd/bootc-fetch-apply-updates.timer.d/10-auros.conf`. The "not synchronised" half rests on the `RandomizedDelaySec=10min` in that file, and has not been measured on a fleet. | `2026-09-20` |
...
| The example recipe's `install_between` window is recorded, not applied | `copy.ts` detailsMono | **DECISION D38**, measured: the field reached the image as nothing, and `auros-base/update-agent/systemd/bootc-fetch-apply-updates.timer.d/10-auros.conf` clears `OnCalendar=` deliberately so a fleet takes a rebuild inside check U1's window at whatever hour it publishes. A quiet window and that requirement are in direct tension and **which wins is §9-reserved**. Until then the site prints the same words `explain` prints — `RECORDED, NOT YET APPLIED` — because showing the time alone advertises a behaviour the machine does not have. | `2026-09-20` |
...
| The console works with JavaScript off | `components/BuildConsole.astro` | **MEASURED, AND NOW MECHANISED (D40)**: `dist/index.html` contains all five pinned runs as native `<details>` elements, every log line in the DOM, and the latest run labelled `last build · 2026-09-20 23:32 UTC`. The live SSE stream is the only part that needs script, and the `<noscript>` says so. Since D40 this is not a one-off hand check: `tools/nojs.mjs` fetches every built page and parses it without executing anything, and fails the build if the words of `<main>` that survive with JavaScript off fall through a committed floor — 5,574 on `/` today — or if any hidden element holds more than 25 words. `tools/prove-red.mjs` moves this page’s argument into a `<template>` on purpose and requires that check to go red. | `2026-09-20` |
| The snapshot stays current | `auros-web/.github/workflows/build-log-snapshot.yml` | **DESIGN, PARTLY UNEVIDENCED.** `auros-web/.github/workflows/build-log-snapshot.yml` re-cuts both files daily and commits when the run they point at has moved. **It has not yet run on a schedule** — it was wired up on 2026-09-20 and its first scheduled execution has not happened. Until it has, "current" is a property of the last manual re-cut. | `2026-09-20` |
...
| The nightly rebuild workflow is in the repository, and CI is free for us because the repositories are public | `pages/5-replaceable-on-purpose.mdx` | **SPEC** §2 and **DECISION D31**, which records free unlimited Actions minutes on public repositories as the actual reason the repositories are public. **DESIGN** for the workflow's own behaviour — `auros-base/.github/workflows/nightly.yml`. Until 2026-09-20 this row backed a sentence inviting the reader to fork that workflow and run it themselves; that sentence is gone, because the licence does not permit it. Verify GitHub's current free-tier terms before publish. | `2026-09-20` | <!-- auros-allow: repository VISIBILITY as a CI cost fact, recorded here and deliberately not published; no licence is offered by this row -->
| There is no Auros account on the image and nothing listening for us | `faq/2-who-has-root.md` | **DESIGN** for "nothing listening": `auros-base/hardening/hardening-assert.sh` fails if sshd is active or firewalld admits an unexpected inbound service. The "no Auros account" half has no check. **BUILD-REQ on workstream A:** add a check-matrix item asserting no vendor account, no authorised key of ours, and no inbound service. Until that check exists this is an intention, not a fact. **This is the most load-bearing trust claim on the site and it should be mechanically proved, not written.** | `2026-09-20` |
